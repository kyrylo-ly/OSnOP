...
class GraphicalEditor(tk.Tk):
    ...
    SUPPORTED_FIGURES = ("line", "rectangle", "ellipse", "image")
    ...
    def _build_menu(self):
        ...
        filters_menu.add_command(label="Rotate 90° CW", command=lambda: self.apply_rotation(90))
        filters_menu.add_command(label="Rotate 90° CCW", command=lambda: self.apply_rotation(-90))
        filters_menu.add_command(label="Flip Horizontal", command=self.apply_flip_horizontal)
        filters_menu.add_command(label="Flip Vertical", command=self.apply_flip_vertical)
    ...
    def _update_window_title(self):
        ...
        self.title(f"Lab10 Graphical Editor - {file_name}{dirty}")
    ...
    def on_mouse_down(self, event):
        ...
        if self.current_figure.get() == "select":
            if self.selected_indices and self._point_in_bbox(cx, cy, self._selection_bbox):
                self._moving = True
    ...
    def on_mouse_move(self, event):
        ...
        if self._pasting:
            self._update_paste_preview(cx, cy)
            return
        ...
        if self.current_figure.get() == "select":
            if self._moving and self._move_start and self._move_origin_shapes is not None:
                dx = cx - self._move_start[0]
                dy = cy - self._move_start[1]
            ...
            if self._selection_rect is not None:
                x1, y1 = self._start
                self.canvas.coords(self._selection_rect, x1, y1, cx, cy)
            return
        ...
        x1, y1 = self._start
        self.canvas.coords(self._preview_item, x1, y1, cx, cy)
    ...
    def _draw_shape(self, shape):
        ...
        elif t == "image" and "pil_image" in shape:
            try:
                photo = ImageTk.PhotoImage(shape["pil_image"])
                self._photo_cache.append(photo)
                self.canvas.create_image(shape["x1"], shape["y1"], image=photo, anchor="nw")
    ...
    def _save_to_path(self, path: Path):
        ...
        for s in self.shapes:
            shape_copy = s.copy()
            if s["type"] == "image" and "pil_image" in s:
                import io
                import base64
                buf = io.BytesIO()
                s["pil_image"].save(buf, format="PNG")
                shape_copy["pil_image_b64"] = base64.b64encode(buf.getvalue()).decode('utf-8')
                del shape_copy["pil_image"]
            serializable_shapes.append(shape_copy)

        payload = {
            "format": "lab10-vector-drawing",
            "version": 1,
        ...
        for s in shapes:
            ...
            try:
                if t == "image":
                    import io
                    import base64
                    if "pil_image_b64" in s:
                        img_data = base64.b64decode(s["pil_image_b64"])
    ...
    def export_image(self):
        ...
        # We need to figure out bounding box of all shapes
        if not self.shapes:
            messagebox.showinfo("Export", "Canvas is empty.")
            return
            
        x1 = min(min(s["x1"], s["x2"]) for s in self.shapes)
        y1 = min(min(s["y1"], s["y2"]) for s in self.shapes)
        x2 = max(max(s["x1"], s["x2"]) for s in self.shapes)
        y2 = max(max(s["y1"], s["y2"]) for s in self.shapes)
        
        # Create a new PIL image to render vector shapes manually, or use Ghostscript wrapper
        # Since Tkinter Canvas to Postscript to PIL is complex, we'll do a simpler approach:
        # Just use Pillow ImageDraw for vector, or just say it's not fully supported for vectors but works for bitmaps.
        # Actually, best way to export canvas in tkinter without GS is using grab.
        try:
            import platform
            if platform.system() == "Darwin":
                # On mac, image grab works nicely
                ...
                messagebox.showwarning("Export", "Export relies on screenshotting the visible window area on non-mac platforms.")
                from PIL import ImageGrab
                x = self.canvas.winfo_rootx()
                y = self.canvas.winfo_rooty()
                w = self.canvas.winfo_width()
    ...
    def _apply_filter_to_selection(self, filter_func, filter_name="Filter"):
        if not self.selected_indices:
            messagebox.showinfo(filter_name, "Select an image object first.")
            return
            
        has_images = any(self.shapes[idx]["type"] == "image" for idx in self.selected_indices)
        if not has_images:
            messagebox.showinfo(filter_name, "Selected objects must include at least one image.")
            return

        self._push_history()
        for idx in self.selected_indices:
            shape = self.shapes[idx]
            if shape["type"] == "image" and "pil_image" in shape:
                # Apply filter and update image
                shape["pil_image"] = filter_func(shape["pil_image"])
                # Recompute width/height if needed
                w, h = shape["pil_image"].size
                shape["x2"] = shape["x1"] + w
                shape["y2"] = shape["y1"] + h
    ...
    def apply_grayscale(self):
        self._apply_filter_to_selection(lambda img: ImageOps.grayscale(img).convert("RGBA"), "Grayscale")
    ...
    def apply_invert(self):
        def invert(img):
            if img.mode == 'RGBA':
                ...
                return ImageOps.invert(img).convert("RGBA")
        self._apply_filter_to_selection(invert, "Invert")

    def apply_sepia(self):
        def sepia(img):
            # Pure python sepia matrix
            ...
            for py in range(height):
                for px in range(width):
                    ...
                    pixels[px, py] = (min(255, tr), min(255, tg), min(255, tb), a)
            return img
        # Need to deepcopy inside lambda to avoid modifying original immediately
        self._apply_filter_to_selection(lambda img: sepia(img.copy()), "Sepia")

    def apply_rotation(self, angle):
        # Image.ROTATE_90 in Pillow is actually CCW by default. 
        # Using rotate(-90) rotates CW.
        self._apply_filter_to_selection(lambda img: img.rotate(angle, expand=True), f"Rotate {angle}")

    def apply_flip_horizontal(self):
        self._apply_filter_to_selection(lambda img: ImageOps.mirror(img), "Flip Horizontal")

    def apply_flip_vertical(self):
        self._apply_filter_to_selection(lambda img: ImageOps.flip(img), "Flip Vertical")
